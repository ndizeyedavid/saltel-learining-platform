...
// Get trainer profile
$trainer_query = "SELECT * FROM users WHERE user_id = ?";
...
// Get assignments created by this trainer for calendar
...
$assignments_stmt->bind_param("i", $trainer_id);
$assignments_stmt->execute();
$upcoming_assignments = $assignments_stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Get courses taught by this trainer
$courses_query = "SELECT c.course_id, c.course_title, c.description, c.created_at
    FROM courses c
    WHERE c.teacher_id = ?
...
$taught_courses = $courses_stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    ...
    <div class="flex h-screen overflow-hidden">
                ...
                <div class="mx-auto max-w-7xl">
                    ...
                    <div class="mb-8">
                        <div class="flex items-center justify-between">
                            <div>
                                <h1 class="mb-2 text-3xl font-bold text-gray-900">Trainer Calendar</h1>
                                <p class="text-gray-600">Manage your course assignments, teaching schedule, and important dates</p>
                            </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-8 lg:grid-cols-3">
                        ...
                        <!-- Teaching Courses -->
                        <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                            <h3 class="mb-4 text-lg font-semibold text-gray-900">Teaching Courses</h3>
                            <div class="space-y-3">
                                <?php if (!empty($taught_courses)): ?>
                                    <?php foreach (array_slice($taught_courses, 0, 5) as $course): ?>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <p class="text-sm text-gray-500">No courses created</p>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-600">Teaching Courses</span>
                                    <span class="font-semibold text-green-600"><?php echo count($taught_courses); ?></span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-600">Teaching Hours</span>
                                    <span class="font-semibold text-blue-600">32h</span>
                                </div>
                                <div class="flex items-center justify-between">
        ...
        <script>
            window.calendarData = {
                ...
                courses: <?php echo json_encode($taught_courses); ?>,
                ...
                function showEventDetails(event) {
                    ...
                    if (props.type === 'assignment') {
                        ...
                        content += `
                            ...
                            <h3 class="mb-2 text-lg font-semibold">${event.title}</h3>
                            <p class="mb-2 text-sm text-gray-600"><strong>Created:</strong> ${event.start.toLocaleDateString()}</p>
                            ${props.description ? `<p class="text-sm text-gray-600"><strong>Description:</strong> ${props.description}</p>` : ''}
                        `;
                    }
